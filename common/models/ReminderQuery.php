...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_reminder" .
 */
class ReminderQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_reminder.id id", "CONCAT_WS(' ', id) text"]);
	}
}