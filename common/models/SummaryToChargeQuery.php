...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_summary_to_charge" .
 */
class SummaryToChargeQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_summary_to_charge.id id", "CONCAT_WS(' ', id) text"]);
	}
}