...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_ticket_to_seat_to_item" .
 */
class TicketToSeatToItemQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_ticket_to_seat_to_item.id id", "CONCAT_WS(' ', id) text"]);
	}
}