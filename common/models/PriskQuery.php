...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_prisk" .
 */
class PriskQuery extends \common\components\ActiveQuery
{
    ...
    public function display($q = null, $page = null)
    {
        ...
        if(is_string($q)) {
            foreach(explode(' ', $q) as $like) {
                ...
                $this->andWhere("CONCAT_WS(' ', first name) LIKE :like", [':like' => "%$like%"]);
            }
        }
        ...
        return parent::display($q, $page)
            ...
            ->select(["tbl_prisk.id id", "CONCAT_WS(' ', first name) text"]);
    }
