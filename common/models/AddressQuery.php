...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_address" .
 */
class AddressQuery extends \common\components\ActiveQuery
{
...
//				$this->andWhere("CONCAT_WS(' ', email, first_name, last_name) LIKE :like", [':like' => "%$like%"]);
				$this->andWhere("CONCAT_WS(' ', address_line1, address_line2, post_code) LIKE :like", [':like' => "%$like%"]);
			}
		}
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_address.id id", "CONCAT_WS(' ', address_line1, address_line2, post_code) text"]);
	}
}