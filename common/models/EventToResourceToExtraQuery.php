...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_event_to_resource_to_extra" .
 */
class EventToResourceToExtraQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_event_to_resource_to_extra.id id", "CONCAT_WS(' ', id) text"]);
	}
}