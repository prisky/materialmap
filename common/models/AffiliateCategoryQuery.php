...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_affiliate_category" .
 */
class AffiliateCategoryQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_affiliate_category.id id", "CONCAT_WS(' ', name) text"]);
	}
}