...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_item_group" .
 */
class ItemGroupQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_item_group.id id", "CONCAT_WS(' ', name) text"]);
	}
}