...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_resource_to_message_to_user" .
 */
class ResourceToMessageToUserQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_resource_to_message_to_user.id id", "CONCAT_WS(' ', id) text"]);
	}
}