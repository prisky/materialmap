...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_cancellation_policy" .
 */
class CancellationPolicyQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_cancellation_policy.id id", "CONCAT_WS(' ', id) text"]);
	}
}