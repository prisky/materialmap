...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_message_field" .
 */
class MessageFieldQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_message_field.id id", "CONCAT_WS(' ', name) text"]);
	}
}