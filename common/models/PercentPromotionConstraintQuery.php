...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_percent_promotion_constraint" .
 */
class PercentPromotionConstraintQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_percent_promotion_constraint.id id", "CONCAT_WS(' ', id) text"]);
	}
}