...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_country" .
 */
class CountryQuery extends \common\components\ActiveQuery
{
...
//				$this->andWhere("CONCAT_WS(' ', email, first_name, last_name) LIKE :like", [':like' => "%$like%"]);
				$this->andWhere("CONCAT_WS(' ', name, phone_prefix, tax_name) LIKE :like", [':like' => "%$like%"]);
			}
		}
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_country.id id", "CONCAT_WS(' ', name, phone_prefix, tax_name) text"]);
	}
}