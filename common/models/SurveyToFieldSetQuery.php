...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_survey_to_field_set" .
 */
class SurveyToFieldSetQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_survey_to_field_set.id id", "CONCAT_WS(' ', id) text"]);
	}
}