...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_resource_type_to_extra" .
 */
class ResourceTypeToExtraQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_resource_type_to_extra.id id", "CONCAT_WS(' ', id) text"]);
	}
}