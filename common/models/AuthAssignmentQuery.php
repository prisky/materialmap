...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_auth_assignment" .
 */
class AuthAssignmentQuery extends \common\components\ActiveQuery
{
    ...
    public function display($q = null, $page = null)
    {
        ...
        if(is_string($q)) {
            foreach(explode(' ', $q) as $like) {
                ...
                $this->andWhere("CONCAT_WS(' ', `item_name`) LIKE :like", [':like' => "%$like%"]);
            }
        }
        ...
        return parent::display($q, $page)
            ...
            ->select(["tbl_auth_assignment.id id", "CONCAT_WS(' ', `item_name`) text"]);
    }
