...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_state_province" .
 */
class StateProvinceQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_state_province.id id", "CONCAT_WS(' ', name) text"]);
	}
}