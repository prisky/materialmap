...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_event_type_to_resource_type_to_extra_to_ticket_to_seat" .
 */
class EventTypeToResourceTypeToExtraToTicketToSeatQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_event_type_to_resource_type_to_extra_to_ticket_to_seat.id id", "CONCAT_WS(' ', id) text"]);
	}
}