...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_sms" .
 */
class SmsQuery extends \common\components\ActiveQuery
{
...
//				$this->andWhere("CONCAT_WS(' ', email, first_name, last_name) LIKE :like", [':like' => "%$like%"]);
				$this->andWhere("CONCAT_WS(' ', sms_message) LIKE :like", [':like' => "%$like%"]);
			}
		}
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_sms.id id", "CONCAT_WS(' ', sms_message) text"]);
	}
}