...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_event_to_resource_to_extra_to_summary" .
 */
class EventToResourceToExtraToSummaryQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_event_to_resource_to_extra_to_summary.id id", "CONCAT_WS(' ', id) text"]);
	}
}