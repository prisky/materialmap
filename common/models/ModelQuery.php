...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_model" .
 */
class ModelQuery extends \common\components\ActiveQuery
{
    ...
    public function display($q = null, $page = null)
    {
        ...
        if(is_string($q)) {
            foreach(explode(' ', $q) as $like) {
                ...
                $this->andWhere("CONCAT_WS(' ', auth_item_name, label, label_plural) LIKE :like", [':like' => "%$like%"]);
            }
        }
        ...
        return parent::display($q, $page)
            ...
            ->select(["tbl_model.id id", "CONCAT_WS(' ', auth_item_name, label, label_plural) text"]);
    }
