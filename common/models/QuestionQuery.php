...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_question" .
 */
class QuestionQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_question.id id", "CONCAT_WS(' ', id) text"]);
	}
}