...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_survey_to_custom_field" .
 */
class SurveyToCustomFieldQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_survey_to_custom_field.id id", "CONCAT_WS(' ', id) text"]);
	}
}