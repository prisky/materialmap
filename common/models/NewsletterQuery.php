...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_newsletter" .
 */
class NewsletterQuery extends \common\components\ActiveQuery
{
...
//				$this->andWhere("CONCAT_WS(' ', email, first_name, last_name) LIKE :like", [':like' => "%$like%"]);
				$this->andWhere("CONCAT_WS(' ', subject) LIKE :like", [':like' => "%$like%"]);
			}
		}
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_newsletter.id id", "CONCAT_WS(' ', subject) text"]);
	}
}