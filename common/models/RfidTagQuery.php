...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_rfid_tag" .
 */
class RfidTagQuery extends \common\components\ActiveQuery
{
    ...
    public function display($q = null, $page = null)
    {
        ...
        if(is_string($q)) {
            foreach(explode(' ', $q) as $like) {
                ...
                $this->andWhere("CONCAT_WS(' ', name_plate, commodity_code) LIKE :like", [':like' => "%$like%"]);
            }
        }
        ...
        return parent::display($q, $page)
            ...
            ->select(["tbl_rfid_tag.id id", "CONCAT_WS(' ', name_plate, commodity_code) text"]);
    }
