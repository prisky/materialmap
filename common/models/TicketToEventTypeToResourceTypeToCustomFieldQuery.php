...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_ticket_to_event_type_to_resource_type_to_custom_field" .
 */
class TicketToEventTypeToResourceTypeToCustomFieldQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_ticket_to_event_type_to_resource_type_to_custom_field.id id", "CONCAT_WS(' ', custom_value) text"]);
	}
}