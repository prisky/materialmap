...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_site_locations" .
 */
class SiteLocationsQuery extends \common\components\ActiveQuery
{
    ...
    public function display($q = null, $page = null)
    {
        ...
        if(is_string($q)) {
            foreach(explode(' ', $q) as $like) {
                ...
                $this->andWhere("CONCAT_WS(' ', name, latitude, longitude) LIKE :like", [':like' => "%$like%"]);
            }
        }
        ...
        return parent::display($q, $page)
            ...
            ->select(["tbl_site_locations.id id", "CONCAT_WS(' ', name, latitude, longitude) text"]);
    }
