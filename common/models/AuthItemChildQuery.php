...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_auth_item_child" .
 */
    ...
    public function display($q = null, $page = null)
    {
        ...
        if(is_string($q)) {
            foreach(explode(' ', $q) as $like) {
                ...
                $this->andWhere("CONCAT_WS(' ', `parent`, `child`) LIKE :like", [':like' => "%$like%"]);
            }
        }
        ...
        return parent::display($q, $page)
            ...
            ->select(["tbl_auth_item_child.id id", "CONCAT_WS(' ', `parent`, `child`) text"]);
    }
