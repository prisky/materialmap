...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_booking_to_event_type_to_resource_type_to_extra" .
 */
class BookingToEventTypeToResourceTypeToExtraQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_booking_to_event_type_to_resource_type_to_extra.id id", "CONCAT_WS(' ', id) text"]);
	}
}