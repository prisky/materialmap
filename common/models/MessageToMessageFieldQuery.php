...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_message_to_message_field" .
 */
class MessageToMessageFieldQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_message_to_message_field.id id", "CONCAT_WS(' ', id) text"]);
	}
}