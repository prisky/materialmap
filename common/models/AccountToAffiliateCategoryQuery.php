...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_account_to_affiliate_category" .
 */
class AccountToAffiliateCategoryQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_account_to_affiliate_category.id id", "CONCAT_WS(' ', id) text"]);
	}
}