...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_mail_queue" .
 */
class MailQueueQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_mail_queue.id id", "CONCAT_WS(' ', subject) text"]);
	}
}