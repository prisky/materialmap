...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_account_to_message_to_user" .
 */
class AccountToMessageToUserQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_account_to_message_to_user.id id", "CONCAT_WS(' ', id) text"]);
	}
}