...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_item_inventory" .
 */
class ItemInventoryQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_item_inventory.id id", "CONCAT_WS(' ', id) text"]);
	}
}