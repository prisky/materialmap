...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_summary_to_account_to_user" .
 */
class SummaryToAccountToUserQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_summary_to_account_to_user.id id", "CONCAT_WS(' ', id) text"]);
	}
}