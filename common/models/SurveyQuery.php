...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_survey" .
 */
class SurveyQuery extends \common\components\ActiveQuery
{
...
//				$this->andWhere("CONCAT_WS(' ', email, first_name, last_name) LIKE :like", [':like' => "%$like%"]);
				$this->andWhere("CONCAT_WS(' ', name, deleted) LIKE :like", [':like' => "%$like%"]);
			}
		}
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_survey.id id", "CONCAT_WS(' ', name, deleted) text"]);
	}
}