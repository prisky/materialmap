...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_bom" .
 */
class BomQuery extends \common\components\ActiveQuery
{
    ...
    public function display($q = null, $page = null)
    {
        ...
        if(is_string($q)) {
            foreach(explode(' ', $q) as $like) {
                ...
                $this->andWhere("CONCAT_WS(' ', `name`, `size1`, `size2`, `wbs`) LIKE :like", [':like' => "%$like%"]);
            }
        }
        ...
        return parent::display($q, $page)
            ...
            ->select(["tbl_bom.id id", "CONCAT_WS(' ', `name`, `size1`, `size2`, `wbs`) text"]);
    }
