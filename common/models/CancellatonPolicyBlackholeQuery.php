...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_cancellaton_policy_blackhole" .
 */
class CancellatonPolicyBlackholeQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_cancellaton_policy_blackhole.id id", "CONCAT_WS(' ', id) text"]);
	}
}