...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_summary_to_percent_promotion" .
 */
class SummaryToPercentPromotionQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_summary_to_percent_promotion.id id", "CONCAT_WS(' ', id) text"]);
	}
}