...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_event_type_to_ticket_type" .
 */
class EventTypeToTicketTypeQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_event_type_to_ticket_type.id id", "CONCAT_WS(' ', id) text"]);
	}
}