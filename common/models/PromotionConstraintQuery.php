...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_promotion_constraint" .
 */
class PromotionConstraintQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_promotion_constraint.id id", "CONCAT_WS(' ', id) text"]);
	}
}