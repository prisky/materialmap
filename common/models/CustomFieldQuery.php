...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_custom_field" .
 */
class CustomFieldQuery extends \common\components\ActiveQuery
{
...
//				$this->andWhere("CONCAT_WS(' ', email, first_name, last_name) LIKE :like", [':like' => "%$like%"]);
				$this->andWhere("CONCAT_WS(' ', label, comment) LIKE :like", [':like' => "%$like%"]);
			}
		}
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_custom_field.id id", "CONCAT_WS(' ', label, comment) text"]);
	}
}