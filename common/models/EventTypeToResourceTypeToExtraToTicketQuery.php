...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_event_type_to_resource_type_to_extra_to_ticket" .
 */
class EventTypeToResourceTypeToExtraToTicketQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_event_type_to_resource_type_to_extra_to_ticket.id id", "CONCAT_WS(' ', id) text"]);
	}
}