...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_field_set_to_item_group" .
 */
class FieldSetToItemGroupQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_field_set_to_item_group.id id", "CONCAT_WS(' ', id) text"]);
	}
}