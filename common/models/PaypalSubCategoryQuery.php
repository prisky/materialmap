...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_paypal_sub_category" .
 */
class PaypalSubCategoryQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_paypal_sub_category.id id", "CONCAT_WS(' ', name) text"]);
	}
}