...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_track" .
 */
class TrackQuery extends \common\components\ActiveQuery
{
    ...
    public function display($q = null, $page = null)
    {
        ...
        if(is_string($q)) {
            foreach(explode(' ', $q) as $like) {
                ...
                $this->andWhere("CONCAT_WS(' ', `longitude`, `latitude`) LIKE :like", [':like' => "%$like%"]);
            }
        }
        ...
        return parent::display($q, $page)
            ...
            ->select(["tbl_track.id id", "CONCAT_WS(' ', `longitude`, `latitude`) text"]);
    }
