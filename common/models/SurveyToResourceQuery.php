...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_survey_to_resource" .
 */
class SurveyToResourceQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_survey_to_resource.id id", "CONCAT_WS(' ', id) text"]);
	}
}