...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_ticket_to_seat_to_event_type_to_resource_type_to_custom_fie" .
 */
class TicketToSeatToEventTypeToResourceTypeToCustomFieldQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_ticket_to_seat_to_event_type_to_resource_type_to_custom_fie.id id", "CONCAT_WS(' ', custom_value) text"]);
	}
}