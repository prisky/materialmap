...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_event_detail" .
 */
class EventDetailQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_event_detail.id id", "CONCAT_WS(' ', name) text"]);
	}
}