...
/**
 * Scopes chained to the find method of an ActiveRecord  for table "tbl_seat_type" .
 */
class SeatTypeQuery extends \common\components\ActiveQuery
{
...
//			->select(["tbl_user.id id", "CONCAT_WS(' ', email, first_name, last_name) text"]);
			->select(["tbl_seat_type.id id", "CONCAT_WS(' ', name) text"]);
	}
}