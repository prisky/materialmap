...
abstract class ActiveRecord extends \yii\db\ActiveRecord
{
    ...
    public static function label($primaryKey = null)
    {
        ...
        if ($primaryKey) {
            ...
            $label = $model->text;
        } elseif (empty($labels[$modelNameShort])) { // otherwise if not cached
    ...
    private static function referencedModelName($attribute)
    {
        ...
        return isset($keyColumnUsage[static::tableName()][$attribute]) ? Inflector::id2camel(preg_replace('/^' . Yii::$app->db->tablePrefix . '/', '', $keyColumnUsage[static::tableName()][$attribute]), '_') : NULL;
    }
    ...
    private function storeImages($attributeName)
    {
        ...
        $this->$attributeName = preg_replace_callback("/src=\"data:([^\"]+)\"/", function ($matches) use ($manager, $options, $path) {
                        ...
                        if (isset($foreignKey[$attributeName])) {
                            ...
                            if ($queryParams) {
                                ...
                                $rowCount = Yii::$app->db->createCommand("
                                    SELECT COUNT(*)
                                    FROM `{$foreignKey[0]}`
                                    WHERE " . implode(' AND ', $whereArray), $queryParams
                                    )->queryScalar();
